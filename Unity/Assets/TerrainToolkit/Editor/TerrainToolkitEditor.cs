...
public class TerrainToolkitEditor : Editor {
	...
	public override void OnInspectorGUI() {
		...
		switch (terrain.toolModeInt) {
			...
			// -------------------------------------------------------------------------------------------------------- TEXTURING TOOLS
			case 2:
			Terrain _ter = (Terrain) terrain.GetComponent(typeof(Terrain));
			if (_ter == null) {
				return;
			}
			TerrainData _terData = _ter.terrainData;
			terrain.splatPrototypes = _terData.splatPrototypes;
			EditorGUILayout.Separator();
			float mouseX;
			EditorGUILayout.BeginHorizontal();
			GUI.skin = terrain.guiSkin;
			...
			foreach (SplatPrototype splatPrototype in terrain.splatPrototypes) {
				...
				if (nTextures == 0) {
					splatPrototype.texture = EditorGUILayout.ObjectField(splatPrototype.texture, typeof(Texture2D), true) as Texture2D;
				} else {
					splatPrototype.texture = EditorGUILayout.ObjectField(splatPrototype.texture, typeof(Texture2D), true) as Texture2D;
				}
				GUI.skin = terrain.guiSkin;
			...
			EditorGUILayout.EndHorizontal();
			if (GUI.changed) {
				_terData.splatPrototypes = terrain.splatPrototypes;
			}
